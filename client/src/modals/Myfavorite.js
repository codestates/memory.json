...
import useMyfavorite from "../components/useMyfavorite";
...
const MyfavoriteArea = styled.div`
  z-index: 999;
  width: 100vmin;
  height: 70%;
  min-height: 400px;
...
const Myfavorite = ({ modalCloser }) => {
  const [page, setPage] = useState(1);
  const [bottom, setBottom] = useState(null);
  const { loading, error, favoriteFeed } = useMyfavorite(page);
